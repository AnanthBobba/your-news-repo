...
# Dedupe by (Company, Title, Link)
before = len(full)
full = full.drop_duplicates(subset=["CompanyName", "NewsDescription",
                                    "ArticleLink"], keep="first")
log(f"Deduped {before} → {len(full)} rows")

# Sort: company alphabetical, then date desc
full = full.sort_values(by=["CompanyName", "PublishDate"],
...
msg = EmailMessage()
msg["Subject"] = f"Daily Company News — {TODAY} ({len(full)} articles, {n_companies} companies)"
msg["From"] = GMAIL_FROM
msg["To"] = EMAIL_TO
msg.set_content(
